...
export default function Burger({ className }) {
	...
	return (
		<button
			...
			className={`relative size-10 rounded-[--radius-icons] bg-[--main-color]
			...
			<div
				...
				<span
					aria-hidden='true'
					className={`absolute block h-0.5 w-4 bg-[--background-color] transition
						ease-linear ${isOpen ? 'w-5 rotate-45' : '-translate-y-1'}`}
				/>
				<span
					aria-hidden='true'
					className={`absolute block h-0.5 w-5 bg-[--background-color] transition
						ease-linear ${isOpen ? 'w-0 opacity-0' : ''}`}
				/>
				<span
					aria-hidden='true'
					className={`absolute block h-0.5 w-4 bg-[--background-color] transition
